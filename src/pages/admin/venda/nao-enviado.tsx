...
export default function VendasEnviadas() {
    ...
    const unsentSales = useQuery(["/api/payment/readUnsent"], async () => {
        const response = await fetch("/api/payment/readUnsent")
    ...
    return (
        <>
            <GoBack />
            {
                unsentSales.isLoading && <Loading />
            }
            <h2 className="text-xl mb-4 text-center">Vendas ainda não enviadas</h2>
            <p className="text-center">Você está visualizando as vendas que ainda não foram processadas.</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 p-4">
                {
                    unsentSales?.data?.map((sale: typeof unsentSales.data[0], i: number) => {
                        return (
                            <div key={i} className="border p-4 rounded-xl">
                                ...
                                <p><span className="font-bold text-gray-600">Código de rastreio:</span> <Link href={`/admin/venda/rastreio/${sale.id}`} className="block bg-red-600 p-2 rounded-xl text-center text-white font-bold hover:bg-red-700">Definir código de rastreio</Link></p>
                            </div>
                        )
                    })
                }
